...
public class cheeseServiceIntegrationTest {
        ...
        void shouldFailUpdateWhenCheeseNotFound() {
                ...
                org.assertj.core.api.Assertions.assertThatThrownBy(() -> cheeseService.updateCheese(999L, dto))
                                .isInstanceOf(IllegalArgumentException.class)
        ...
        void shouldFailUpdateWhenNameAlreadyExists() {
                ...
                org.assertj.core.api.Assertions.assertThatThrownBy(() -> cheeseService.updateCheese(c1.getId(), dto))
                                .isInstanceOf(IllegalArgumentException.class)
                                .hasMessage("Cheese with that name already exists");
        }
        ...
        @Test
        void shouldAddBoxSuccessfully() {
                Cheese cheese = cheeseRepository.save(
                                new Cheese(null, "AddBox", 10.0, "d", "t", "2024-01-01", "2025-01-01"));

                CheeseDTO updated = cheeseService.addBox(cheese.getId(), 3.5);

                assertThat(updated.boxes()).containsExactly(3.5);

                Cheese saved = cheeseRepository.findById(cheese.getId()).get();
                assertThat(saved.getBoxes()).containsExactly(3.5);
        }

        @Test
        void shouldAddBoxWhenListIsNull() {
                Cheese cheese = new Cheese(null, "AddBoxNull", 10.0, "d", "t", "2024-01-01", "2025-01-01");
                cheese.setBoxes(null);
                cheese = cheeseRepository.save(cheese);

                CheeseDTO updated = cheeseService.addBox(cheese.getId(), 2.0);

                assertThat(updated.boxes()).containsExactly(2.0);

                Cheese saved = cheeseRepository.findById(cheese.getId()).get();
                assertThat(saved.getBoxes()).containsExactly(2.0);
        }

        @Test
        void shouldFailAddBoxWhenCheeseNotFound() {
                org.assertj.core.api.Assertions.assertThatThrownBy(() -> cheeseService.addBox(999L, 1.0))
                                .isInstanceOf(IllegalArgumentException.class)
                                .hasMessage("Cheese not found");
        }

        @Test
        void shouldRemoveBoxSuccessfully() {
                Cheese cheese = new Cheese(null, "RemoveBox", 10.0, "d", "t", "2024-01-01", "2025-01-01");
                cheese.setBoxes(new java.util.ArrayList<>(List.of(1.0, 2.0, 3.0)));
                cheese = cheeseRepository.save(cheese);

                CheeseDTO updated = cheeseService.removeBox(cheese.getId(), 1);

                assertThat(updated.boxes()).containsExactly(1.0, 3.0);

                Cheese saved = cheeseRepository.findById(cheese.getId()).get();
                assertThat(saved.getBoxes()).containsExactly(1.0, 3.0);
        }

        @Test
        void shouldFailRemoveBoxWhenCheeseNotFound() {
                org.assertj.core.api.Assertions.assertThatThrownBy(() -> cheeseService.removeBox(999L, 0))
                                .isInstanceOf(IllegalArgumentException.class)
                                .hasMessage("Cheese not found");
        }

        @Test
        void shouldFailRemoveBoxWhenNoBoxes() {
                Cheese cheese = new Cheese(null, "NoBoxes", 10.0, "d", "t", "2024-01-01", "2025-01-01");
                cheese.setBoxes(new java.util.ArrayList<>());
                Cheese saved = cheeseRepository.save(cheese);

                org.assertj.core.api.Assertions.assertThatThrownBy(() -> cheeseService.removeBox(saved.getId(), 0))
                                .isInstanceOf(IllegalArgumentException.class)
                                .hasMessage("No boxes to remove");
        }

        @Test
        void shouldFailRemoveBoxWhenIndexInvalid() {
                Cheese cheese = new Cheese(null, "BadIndex", 10.0, "d", "t", "2024-01-01", "2025-01-01");
                cheese.setBoxes(new java.util.ArrayList<>(List.of(1.0)));
                Cheese saved = cheeseRepository.save(cheese);

                org.assertj.core.api.Assertions.assertThatThrownBy(() -> cheeseService.removeBox(saved.getId(), 5))
                                .isInstanceOf(IllegalArgumentException.class)
